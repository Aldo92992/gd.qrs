...
exports.list = async(entityName, filter = '') => {
    ...
    let path = '/' + lEntityName + '/full';
    if (filter !== '') filter = '&filter=' + filter;
    let data = await http.get(path, filter);
    return (data.data);
};

/*exports.getById = async (entityName, id, filter = '') => {
    let lEntityName = entityName.toLowerCase();
    if (allowedEntities.indexOf(lEntityName) == -1) {
        throw new Error('Generic entity ' + entityName + ' is not allowed.');
    }
    let path = '/' + lEntityName + '/' + id;
    if (filter !== '') filter = '&filter=' + filter;
    let data = await http.get(path, filter);
    return (data.data);
};

exports.create = async (entityName, data, filter = '') => {
    let lEntityName = entityName.toLowerCase();
    if (allowedEntities.indexOf(lEntityName) == -1) {
        throw new Error('Generic entity ' + entityName + ' is not allowed.');
    }
    ...
    if (filter !== '') filter = '&filter=' + filter;
    let dt = await http.post(path, data, filter);
    return (dt.data);
};

exports.edit = async (entityName, id, data, filter = '') => {
    let lEntityName = entityName.toLowerCase();
    if (allowedEntities.indexOf(lEntityName) == -1) {
        throw new Error('Generic entity ' + entityName + ' is not allowed.');
    }
    let path = '/' + lEntityName + '/' + id;
    if (filter !== '') filter = '&filter=' + filter;
    let dt = await http.put(path, data, filter);
    return (dt.data);
};

exports.delete = async (entityName, id, filter = '') => {
    let lEntityName = entityName.toLowerCase();
    if (allowedEntities.indexOf(lEntityName) == -1) {
        throw new Error('Generic entity ' + entityName + ' is not allowed.');
    }
    let path = '/' + lEntityName + '/' + id;
    if (filter !== '') filter = '&filter=' + filter;
    let dt = await http.delete(path, filter);
    return (dt.data);
};*/